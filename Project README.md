...
├── models/  
│   ├── model\_configs/          \# Hyperparameter configurations  
│   └── architecture\_notes.md   \# Detailed logic for model selection  
├── src/  
│   ├── preprocessing\_logic.py  \# Methodology for data cleaning (Code snippets only)  
│   └── evaluation\_metrics.py   \# Custom metric implementations  
├── README.md  
└── results\_summary.txt         \# High-level performance comparison (No raw data)
